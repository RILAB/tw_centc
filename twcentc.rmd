...
Standardize matrix and do PC
```{r}
made_x<-Mmake(x)
pc<-prcomp(made_x)  
```

Do Tracy Widom (figure out this shit later). k is number of PCs to evaluate. Here, trying 150
```{r}
TW<-TWcalc(made_x,150) 
km<-which(TW[[2]]>0.05)[1]
```

Cluster individuals based on first km significant PCs
```{r}
